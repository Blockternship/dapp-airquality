...
pub mod android {
  ...
  #[no_mangle]
  pub unsafe extern fn Java_canaries_kike_pocsign_MainActivity_ethkeyBrainwalletSecret(env: JNIEnv, _: JClass, seed: JString) -> jstring {
    let seed: String = env.get_string(seed).expect("Invalid seed").into();
    let keypair = Brain::new(seed).generate().unwrap();
    let java_secret = env.new_string(format!("{:?}", keypair.secret())).expect("Could not create java string");
    java_secret.into_inner()
  ...
  #[no_mangle]
  pub unsafe extern fn Java_canaries_kike_pocsign_MainActivity_ethkeyBrainwalletSign(env: JNIEnv, _: JClass, seed: JString, message: JString) -> jstring {
    let seed: String = env.get_string(seed).expect("Invalid seed").into();
    let message: String = env.get_string(message).expect("Invalid message").into();
    let keypair = Brain::new(seed).generate().unwrap();
    let message: Message = message.parse().unwrap();
  ...
  #[no_mangle]
  pub unsafe extern fn Java_canaries_kike_pocsign_MainActivity_ethkeyKeccak(env: JNIEnv, _: JClass, data: JString) -> jstring {
    let data: String = env.get_string(data).expect("Invalid seed").into();
    let hex = data.from_hex().unwrap();
    let mut res: [u8; 32] = [0; 32];
    let mut keccak = Keccak::new_keccak256();
  ...
  #[no_mangle]
  pub unsafe extern fn Java_canaries_kike_pocsign_MainActivity_ethkeyEthSign(env: JNIEnv, _: JClass, data: JString) -> jstring {
    let data: String = env.get_string(data).expect("Invalid seed").into();
    let hex = data.from_hex().unwrap();
    let message = format!("\x19Ethereum Signed Message:\n{}", hex.len()).into_bytes();
    let mut res: [u8; 32] = [0; 32];
  ...
  #[no_mangle]
  pub unsafe extern fn Java_canaries_kike_pocsign_MainActivity_ethkeyBlockiesIcon(env: JNIEnv, _: JClass, seed: JString) -> jstring {
    let seed: String = env.get_string(seed).expect("Invalid seed").into();
    let icon = blockies_icon_in_base64(seed.into());
    env.new_string(icon).expect("Could not create java string").into_inner()
  }
  ...
  #[no_mangle]
  pub unsafe extern fn Java_canaries_kike_pocsign_MainActivity_ethkeyRandomPhrase(env: JNIEnv, _: JClass, words: jint) -> jstring {
    let words = wordlist::random_phrase(words as usize);
    env.new_string(words).expect("Could not create java string").into_inner()
  }

  #[no_mangle]
  pub unsafe extern fn Java_canaries_kike_pocsign_MainActivity_ethkeyEncryptData(env: JNIEnv, _: JClass, data: JString, password: JString) -> jstring {
    let data: String = env.get_string(data).expect("Invalid data").into();
    let password: String = env.get_string(password).expect("Invalid password").into();
    let crypto = Crypto::with_plain(data.as_bytes(), &password, 10240);
    env.new_string(String::from(crypto)).expect("Could not create java string").into_inner()
  ...
  #[no_mangle]
  pub unsafe extern fn Java_canaries_kike_pocsign_MainActivity_ethkeyDecryptData(env: JNIEnv, _: JClass, data: JString, password: JString) -> jstring {
    let data: String = env.get_string(data).expect("Invalid data").into();
    let password: String = env.get_string(password).expect("Invalid password").into();
    let crypto: Crypto = match data.parse() {
      Ok(crypto) => crypto,
  ...
  mod tests {
    ...
    use ethstore::Crypto;
    use super::Java_canaries_kike_pocsign_MainActivity_ethkeyDecryptData;
    ...
    fn test_decrypt() {
      unsafe {
        ...
        let result = Java_canaries_kike_pocsign_MainActivity_ethkeyDecryptData(
          jvm.env(),
          any_class,
          jni_crypto_str,
          jni_password_str
        ...
        let _ = Java_canaries_kike_pocsign_MainActivity_ethkeyDecryptData(
          jvm.env(),
          any_class,
          jni_crypto_str,
